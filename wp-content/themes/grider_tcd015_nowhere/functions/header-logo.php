...
echo '<div class="block" id="logo_block_text">' ."\n";
echo '<h1 id="logo"><a href="'.home_url().'/">'.get_bloginfo('name')."</a></h1>\n";
echo '<h2 id="description">'. get_bloginfo('description') ."</h2>\n";
echo '</div>' ."\n";
}
}
...
function _dp_delete_image(){
	if(isset($_REQUEST['page'], $_REQUEST['_wpnonce']) && !isset($_REQUEST['settings-updated']) && $_REQUEST['page'] == 'theme_options'){
		...
		$options = get_desing_plus_option();
			...
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_single_ad_image4')){
				$file = basename($options['single_ad_image4']);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
					$options['single_ad_image4'] = '';
					update_option('dp_options', $options);
					add_action('admin_notices', '_dp_delete_message_sucess');
				}else{
					add_action('admin_notices', '_dp_delete_message_error');
		...
		for($i = 1; $i <= 3; $i++){
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_post_ad_'.$i)){
				$file = basename($options['post_ad_image'.$i]);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
					$options['post_ad_image'.$i] = '';
					update_option('dp_options', $options);
					add_action('admin_notices', '_dp_delete_message_sucess');
				}else{
					add_action('admin_notices', '_dp_delete_message_error');
		...
		for($i = 1; $i <= 10; $i++){
			...
			if(wp_verify_nonce($_REQUEST['_wpnonce'], 'dp_delete_original_pattern')){
				$file = basename($options['original_pattern']);
				if(file_exists(dp_logo_basedir().DIRECTORY_SEPARATOR.$file) && @unlink(dp_logo_basedir().DIRECTORY_SEPARATOR.$file)){
